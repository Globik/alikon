...
var ROOM=100
var PEER=100
const {tjson}=require('./unter_websock.js')
...
if(r.rows.length > 0)sendback(ws,{type:'history',d:r.rows})
...
emergency_to_all_in_room(ws,tjson({type:"stat_room",peers:peerlength,room_name:roomname}))
...
ws.roomid=mn;
inserting_room(mn)
sendback(ws,{roomname:mn,type:'onroom'})
...
console.log(e)
sendback(ws,{type:"error", ename:e.name,emsg:e.message})
delete ws.roomid;
}
}
...
if(err){
sendback(ws,{type:'error',error:err,roomname: urlroom})
}
})
}	
}
const list_of_clients=(ws,obj,bool,level,db)=>{
	//console.log('wssssssssssssssssssssssssssssssssssssssssssssssssssssssssss: ',ws.upgradeReq.url)
...
as(item, async function(el){
	//console.log('what the fuck',ws.upgradeReq.url)
if(level=="uroom"){
	//console.log('uuuuuuuuuuuuuuuuuuuuuuuuuuuroooooooooooooooooooooooooooooooooooooooooooooooooooom: ',ws.upgradeReq.url)
if(el.upgradeReq.url===ws.upgradeReq.url){
	//console.log('eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeliiiiiiiiiiiiiiiiii:',el.upgradeReq.url)
if(el && el.readyState===1){
	//console.log('is OPENNNNNNNNNNNNNNNNNNNNNNNNN!!! BOOOOOOOOOOOOOOOOOOOOOOL',bool,obj)
if(bool){
el.send(bmess);rsn++;console.log('truuuuuuuuuuuuuuuuuuuuu')
}else{
//console.log('falseeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee')
if(el.ready===true){
sifilis=true
pidment=el.pidi
...
//let sifa=await
let msg={type:"joined_user",mus_cnt:b.arr.length,username:username,clid:clid,users:b.lusers,ready:b.sifilis,pidi:b.pidment,info:b.info,chataccess:b.chataccess}
...
try{return await promis_list('fake',{fake:'f'},'fake',"websock")}catch(e){console.log(e)}
}
...
ws.owner=msg.owner
ws.ready=false
send_new_user_to_all(ws,msg.name,msg.id)
sendtoclients=false;
}else if(msg.type=="emergency_stop"){
super_send(ws,{type:"emergency_stop",msg:"Stop all streams"});
sendtoclients=false;
}else if(msg.type=="note"){
super_send(ws,{type:"message",msg:msg.msg,from_nick:msg.from_nick})
sendtoclients=false;
...
if(msg.owner){
if(droom.size > ROOM) {
sendback(ws,{type:"overfilled", roomquan:ROOM,roomsize:droom.size,emsg:"Rooms size is overfilled. Please wait some time",etype:5})
return;
}
...
sendback(ws,{type:"error",ename:"Mediasoup is closed!",emsg:"Are you online?"})
}
sendtoclients=false;
...
emergency_to_all_in_room(ws,tjson({type:'roomer_online',ready:true,pidi:msg.pidi,src:res.rows[0].src,chataccess:msg.chataccess}));
sse.publish('ch_log_rooms','add_room', res.rows[0])
//status,view,room_name,src
}).catch(er=>{console.log(er);
emergency_to_all_in_room(ws,tjson({type:'roomer_online',ready:true,pidi:msg.pidi,src:undefined}))			 
})
sendtoclients=false;
...
update_end(msg.pidi)
emergency_to_all_in_room(ws,tjson({type:'roomer_offline',ready:false,pidi:0}))
sse.publish('ch_log_rooms','remove_room', {room_name:msg.roomname})
}).catch(e=>{console.log(e)})
sendtoclients=false;
}else if(msg.type=="money_trans"){
emergency_to_all_in_room(ws,tjson({type:'token_antwort',from:msg.from,to:msg.to,amount:msg.amount,btype:msg.btype,pid:msg.pid,user_nick:msg.from_nick}))
...
send_to_one_user_in_room(ws,msg.target,msg);
}else{
if(msg.type=='message'){
...
emergency_to_all_in_room(ws,jsob)
}
}
//})
}
...
const sock_onclose=(ws,pool,urlRoom)=>{
console.log('WEBSOCKET CLOSED')
	//function on_leave_out(wss,ws,id,name)
on_leave_out(ws,ws.clientId,ws.username)

...
perform pg_notify('bitaps_ok',json_build_object('us_id',bitaps_cb_proc.cb_us_id,'items',p_pack,'type','paid')::text);
	...
	perform pg_notify('bitaps_ok',json_build_object('us_id',bitaps_cb_proc.cb_us_id,'items',p_pack,'type','confirming')::text);
	-- if p_pack = 0 then
	-- update busers set busers.dolg=busers.dolg + bitaps_cb_proc.amt where busers.id=bitaps_cb_proc.cb_us_id;
	-- end if;
	update bitaps_temp set bt_status='confirming',bt_l_mod=now() where bitaps_temp.p_c=bitaps_cb_proc.p_c;
	elsif muck = 3 then
	update bitaps_temp set bt_status='ok',bt_l_mod=now() where bitaps_temp.p_c=bitaps_cb_proc.p_c;
	else
